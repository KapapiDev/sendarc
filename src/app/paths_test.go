...
func TestAppDataDir_EnvPrecedence(t *testing.T) {
	t.Run("SENDARC_APPDATA_DIR wins", func(t *testing.T) {
		tmp := t.TempDir()
		t.Setenv("SENDARC_APPDATA_DIR", tmp)
		t.Setenv("APPDATA", "C:\\Users\\other\\AppData\\Roaming")
		got := appDataDir()
		if got != tmp {
			t.Errorf("SENDARC_APPDATA_DIR should win: got %q, want %q", got, tmp)
		}
	})
	t.Run("APPDATA used when override empty", func(t *testing.T) {
		t.Setenv("SENDARC_APPDATA_DIR", "")
		t.Setenv("APPDATA", "C:\\Users\\test\\AppData\\Roaming")
		got := appDataDir()
		want := filepath.Join("C:\\Users\\test\\AppData\\Roaming", "SendArc")
		if got != want {
...
// to ignore TEMP/TMP entirely and resolve via %LOCALAPPDATA%\SendArc\queue to
// match the DLL-side SHGetFolderPathW(CSIDL_LOCAL_APPDATA) path.
//
//  1. SENDARC_WATCH_DIR        → used as-is (test / per-session override)
//  2. %LOCALAPPDATA%\SendArc\queue
...
func TestWatcherDir_EnvPrecedence(t *testing.T) {
	...
	t.Run("SENDARC_WATCH_DIR wins — used as-is", func(t *testing.T) {
		t.Setenv("SENDARC_WATCH_DIR", overrideDir)
	...
	t.Run("LOCALAPPDATA used when SENDARC_WATCH_DIR empty — TEMP and TMP ignored", func(t *testing.T) {
		t.Setenv("SENDARC_WATCH_DIR", "")
		t.Setenv("LOCALAPPDATA", localAppData)
		...
		want := filepath.Join(localAppData, "SendArc", "queue")
		got := watcherDir()
		if got != want {
	...
	t.Run("non-empty fallback when LOCALAPPDATA and SENDARC_WATCH_DIR both empty", func(t *testing.T) {
		t.Setenv("SENDARC_WATCH_DIR", "")
...
func TestUpdatesStagingDir_EnvPrecedence(t *testing.T) {
	t.Run("SENDARC_UPDATES_DIR wins", func(t *testing.T) {
		tmp := t.TempDir()
		t.Setenv("SENDARC_UPDATES_DIR", tmp)
		t.Setenv("ProgramData", `C:\OtherProgramData`)
		got := updatesStagingDir()
		if got != tmp {
			t.Errorf("SENDARC_UPDATES_DIR should win: got %q, want %q", got, tmp)
		}
	})
	t.Run("ProgramData used when override empty", func(t *testing.T) {
		t.Setenv("SENDARC_UPDATES_DIR", "")
		t.Setenv("ProgramData", `C:\ProgramData`)
		got := updatesStagingDir()
		want := filepath.Join(`C:\ProgramData`, "SendArc", "updates")
		if got != want {
	...
	t.Run("fallback when no env", func(t *testing.T) {
		...
		got := updatesStagingDir()
		// Just assert it ends with SendArc/updates and is rooted in TempDir.
		wantSuffix := filepath.Join("SendArc", "updates")
