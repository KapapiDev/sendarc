...
func showWebView2MissingDialog() {
	...
	title, _ := syscall.UTF16PtrFromString("SendArc — WebView2 required")
	body, _ := syscall.UTF16PtrFromString(
		"Microsoft Edge WebView2 Runtime is required to run SendArc.\r\n\r\n" +
			"Your system browser will now open the Microsoft download page. " +
			"Install the runtime, then relaunch SendArc.")
