...
// used by local development: SENDARC_OAUTH_CLIENT_ID, SENDARC_OAUTH_CLIENT_SECRET.
func init() {
	if v := os.Getenv("SENDARC_OAUTH_CLIENT_ID"); v != "" {
		oauthClientID = v
	}
	if v := os.Getenv("SENDARC_OAUTH_CLIENT_SECRET"); v != "" {
		oauthClientSecret = v
	}
}
