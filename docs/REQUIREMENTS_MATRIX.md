...
| Single-account connect/disconnect/re-auth | Partially verified with real account | A production-mode build completed Google consent on 2026-09-01, stored the token set in Windows Credential Manager, restored the signed-in state after restart, and passed live Google token introspection | Force token refresh and verify disconnect/reconnect in final acceptance; account identity remains intentionally generic without profile scopes |
| Polished Status/Account/Settings/About functions | Partially verified | Real Gmail/MAPI/component/default-handler status, privacy-safe activity timestamps, Google token-introspection test, guarded elevated MAPI repair, auth/diagnostics/update controls, and About/privacy/license/support are wired. The Status and About modals passed type/accessibility, small-window scroll, keyboard focus, and Escape/focus-return Windows checks; the real Google connection test passed on 2026-09-01 | Complete installed UAC repair/refresh once in final Windows acceptance |
...
| Real Google OAuth + Gmail Sent | In progress; sign-in and credential persistence verified | Project/API, branding/policies, external Testing audience, controlled test user, exact scope, Desktop client, and protected injection are complete. On 2026-09-01 a production-mode Windows build completed real consent, stored the token set in Windows Credential Manager, restored it after restart, and passed live Google token introspection | Prove forced refresh and disconnect/reconnect, then perform the explicit sanitized send and verify the result in Gmail Sent |
