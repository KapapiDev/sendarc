...
| Attachment/path/header validation | Verified | Go tests reject header injection, unsafe names, filename/path mismatch, and attachments outside the queue-owned sibling directory; C++ tests reject traversal, absolute paths, and separators while preserving Unicode basenames. x86/x64 CTest is green locally and in [run 33283875228](https://github.com/maxtop9843-byte/sendarc/actions/runs/33283875228) | Repeat at the immutable release tag |
| Privacy-safe errors/logging/diagnostics | Implemented; verification pending | Typed Gmail errors, generic frontend errors, sanitized ID logging, [SECURITY.md](../SECURITY.md); Windows E2E inspects Gmail 503 and offline logs and rejects subject, recipient, token, and response-detail leakage | Reconfirm at the native E2E CI head and inspect real-auth failure logs |
...
| Windows install/MAPI/uninstall smoke | Implemented; verification pending | Current x64/x86 interceptor CI and installer Pester round-trip are green; a local x64 native `MAPISendMailW` call reached the real Wails preview with Unicode fields and an attachment on 2026-08-30 | Installed third-party legacy-app call, Gmail Sent, and interactive uninstall/restore remain |
...
| Requirement | Status | Evidence | Gap / release gate |
|---|---|---|---|
| Current app/frontend tests | Verified at pushed head | [Build and Test run 33283875228](https://github.com/maxtop9843-byte/sendarc/actions/runs/33283875228) passed Go vet, both Go race suites, the notification-action safety regression, Svelte check, 77 frontend tests, Wails build, seven Windows app E2E scenarios, and x86/x64 Debug/Release interceptor matrices at `47f6117` | Repeat for the native-MAPI E2E commit and at the immutable release candidate/tag |
| Windows app E2E | Verified locally and in CI | Seven real Wails/WebView2 Playwright scenarios passed locally on 2026-08-30, now beginning with an actual x64 `MAPISendMailW` call: native interception/local preview, explicit send with complete MIME, Cancel/Dismiss, multi-arrival, Gmail 503 retry, offline queue retention, and repeated-401 re-auth using hermetic local OAuth/Gmail services. The expanded pre-native suite passed in [run 33283875228](https://github.com/maxtop9843-byte/sendarc/actions/runs/33283875228) | Confirm the native seven-scenario suite at the next pushed head and repeat at the immutable release candidate |
| C++/installer CI | Verified at pushed head | x64/x86 Debug/Release jobs passed in [run 33283875228](https://github.com/maxtop9843-byte/sendarc/actions/runs/33283875228); 27-test Pester install/uninstall round-trip including version metadata, AUMID, license bundle, coexistence, and previous-handler restoration passed in [run 33283875259](https://github.com/maxtop9843-byte/sendarc/actions/runs/33283875259) | Repeat at the immutable release candidate/tag and in final interactive acceptance |
| Website CI/QA | Verified | [Website run 33283875184](https://github.com/maxtop9843-byte/sendarc/actions/runs/33283875184) passed at `47f6117`; 20 live cross-viewport Playwright/axe scenarios also pass on the deployed site | Repeat after the release asset changes download state |
| Dependency/secret/security audit | Verified at pushed head | [Security run 33283875237](https://github.com/maxtop9843-byte/sendarc/actions/runs/33283875237) reproduced the checked-in dependency inventories and passed Gitleaks at `47f6117`; both npm audits report 0 vulnerabilities, both Go modules report 0 reachable vulnerabilities, and the 595-commit history scan is clean; [SECURITY_AUDIT.md](SECURITY_AUDIT.md) | Repeat against the native-MAPI E2E commit and immutable release tag |
