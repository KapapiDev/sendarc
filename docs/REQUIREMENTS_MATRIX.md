...
| Requirement | Status | Evidence | Gap / release gate |
|---|---|---|---|
| Current app/frontend tests | Verified at pushed head | [Build and Test run 33284768883](https://github.com/maxtop9843-byte/sendarc/actions/runs/33284768883) passed Go vet, both Go race suites, the isolated real Credential Manager integration, Svelte check, 77 frontend tests, Wails build, the native-MAPI seven-scenario Windows E2E, all x86/x64 Debug/Release interceptor matrices, and the production-DLL test-hook isolation check at `fc5e140` | Repeat at the immutable release candidate/tag |
...
| C++/installer CI | Verified at pushed head | x64/x86 Debug/Release jobs passed in [run 33284768883](https://github.com/maxtop9843-byte/sendarc/actions/runs/33284768883); the 27-test Pester install/uninstall round-trip including version metadata, AUMID, license bundle, coexistence, and previous-handler restoration passed in [run 33284768842](https://github.com/maxtop9843-byte/sendarc/actions/runs/33284768842) | Repeat at the immutable release candidate/tag and in final interactive acceptance |
| Website CI/QA | Verified | [Website run 33284768864](https://github.com/maxtop9843-byte/sendarc/actions/runs/33284768864) passed at `fc5e140`; 20 live cross-viewport Playwright/axe scenarios also pass on the deployed site | Repeat after the release asset changes download state |
| Dependency/secret/security audit | Verified at pushed head | [Security run 33284768867](https://github.com/maxtop9843-byte/sendarc/actions/runs/33284768867) reproduced the checked-in dependency inventories and passed Gitleaks at `fc5e140`; both npm audits report 0 vulnerabilities, both Go modules report 0 reachable vulnerabilities, and the 595-commit history scan is clean; [SECURITY_AUDIT.md](SECURITY_AUDIT.md) | Repeat at the immutable release tag |
