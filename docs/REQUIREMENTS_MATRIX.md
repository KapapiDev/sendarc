...
| Coexistence; never remove unrelated mail apps | Implemented; verification pending | The Pester round-trip plants Affixa, go-mapi, and an alternate default client; asserts install preservation, JSON plus machine-owned registry backup, uninstall restoration, and non-removal | Confirm the current restore path in Windows CI and repeat in final interactive acceptance |
