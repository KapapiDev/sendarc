...
Last audited: 2026-09-02 against `SENDARC_AUTONOMOUS_A_TO_Z_BUILD_PROMPT.md`, the current worktree, GitHub repository state, and the conversation overrides.
...
|---|---|---|---|
| Public SendArc repository | Verified | [KapapiDev/sendarc](https://github.com/KapapiDev/sendarc) is public; current work is pushed in PR #1 | Merge only after the current clean CI and release gates pass |
...
| KaPaPi parent-brand relationship | Implemented; deployment verification pending | SendArc remains the product name and uses “A KaPaPi product”; `kapapi.dev/sendarc/` is the product-family entry path, with the root card/page implemented in the separate KaPaPi site branch | Deploy the KaPaPi site change and verify the public path without replacing the full Cloudflare application site |
...
| Requirement | Status | Evidence | Gap / release gate |
|---|---|---|---|
| Gmail `users.messages.send` transport | Implemented; verification pending | `internal/mapi/gmail.go` uses `/messages/send`; request/error tests plus the real Wails/WebView2 hermetic E2E in [run 33284510085](https://github.com/KapapiDev/sendarc/actions/runs/33284510085) observed exactly one `/messages/send` request and zero draft attempts after a native `MAPISendMailW` call | Repeat at the immutable release candidate and obtain a real Gmail Sent result |
...
| Single-account connect/disconnect/re-auth | Partially verified with real account | A production-mode build completed Google consent on 2026-09-01, stored the token set in Windows Credential Manager, restored the signed-in state after restart, and passed live Google token introspection. After natural token expiry, the 2026-09-02 connection test completed a real Google refresh, persisted the new expiry, and restored it after restart | Verify disconnect/reconnect in final acceptance; account identity remains intentionally generic without profile scopes |
...
| Clean checkout desktop build | Verified | GitHub Actions [run 33531293956](https://github.com/KapapiDev/sendarc/actions/runs/33531293956) passed the Wails app and all interceptor matrices at `5e526bc` on `windows-2025` | Repeat at the immutable release tag |

## Installer, updates, and Windows validation

| Requirement | Status | Evidence | Gap / release gate |
|---|---|---|---|
| x64 and x86 interceptor builds | Verified | Latest-head [Build and Test run 33531293956](https://github.com/KapapiDev/sendarc/actions/runs/33531293956) passed x64/x86 Debug/Release builds and harness tests; [release dry-run 33532008202](https://github.com/KapapiDev/sendarc/actions/runs/33532008202) rebuilt both Release targets with tests | Repeat at the immutable release tag |
| NSIS installer with SendArc paths/registry/uninstall entry | Verified by automated round-trip | `src/installer/SendArc.nsi`; latest-head [installer run 33531293901](https://github.com/KapapiDev/sendarc/actions/runs/33531293901) and exact-artifact [release dry-run 33532008202](https://github.com/KapapiDev/sendarc/actions/runs/33532008202) passed all 31 install/uninstall cases | Perform the final interactive clean-Windows acceptance flow |
| Preserve/restore previous mail handler | Verified by automated round-trip | [Installer run 33531293901](https://github.com/KapapiDev/sendarc/actions/runs/33531293901) exercises JSON plus registry backup, repeated install/uninstall, and prior-handler restoration | Confirm once more in the final interactive acceptance flow |
| Coexistence; never remove unrelated mail apps | Verified by automated round-trip | [Installer run 33531293901](https://github.com/KapapiDev/sendarc/actions/runs/33531293901) plants Affixa, go-mapi, and an alternate default; verifies install preservation, restoration, and non-removal through repeated cycles | Repeat in final interactive acceptance |
...
| Windows install/MAPI/uninstall smoke | Verified by automated installed-system round-trip | [Installer run 33531293901](https://github.com/KapapiDev/sendarc/actions/runs/33531293901) and exact-artifact [release dry-run 33532008202](https://github.com/KapapiDev/sendarc/actions/runs/33532008202) pass installed x64/x86 `MAPISendMail` and `MAPISendMailW`, version/AUMID/license/coexistence, reinstall, uninstall, and previous-handler restore in 31/31 cases | Installed third-party legacy-app call, Gmail Sent, and interactive uninstall/restore remain |
...
| Public Cloudflare deployment | Verified | Production deployment `5464a9c8-81ea-4e3a-9761-be69f086af63` is live at `https://sendarc.pages.dev`; Functions bundle and D1 binding are active; 20 live cross-viewport tests passed on 2026-09-02 | Custom domain remains deferred |
| Product-family domain path | Implemented; deployment verification pending | Owner operates `kapapi.dev`; [DECISIONS.md](../DECISIONS.md) records `/sendarc/` under the KaPaPi umbrella while `SendArc.app` remains deferred | Deploy and verify `https://kapapi.dev/sendarc/`; keep the full Pages Functions site on `sendarc.pages.dev` until path-aware migration is proven |
...
| Requirement | Status | Evidence | Gap / release gate |
|---|---|---|---|
| Current app/frontend tests | Verified at pushed head | [Build and Test run 33531293956](https://github.com/KapapiDev/sendarc/actions/runs/33531293956) passed Go vet/race suites, isolated real Credential Manager integration, Svelte checks and 77 frontend tests, Wails build, native-MAPI Windows E2E, all x86/x64 Debug/Release matrices, the manual attachment-queue contract, and production-DLL test-hook isolation at `5e526bc` | Repeat at the immutable release tag |
...
| C++/installer CI | Verified at pushed head | x64/x86 Debug/Release jobs passed in [run 33531293956](https://github.com/KapapiDev/sendarc/actions/runs/33531293956); the 31-test installed-system Pester round-trip passed in [run 33531293901](https://github.com/KapapiDev/sendarc/actions/runs/33531293901) and again against the exact release bytes in [run 33532008202](https://github.com/KapapiDev/sendarc/actions/runs/33532008202) | Repeat at the immutable release tag and in final interactive acceptance |
| Website CI/QA | Verified | [Website run 33531294046](https://github.com/KapapiDev/sendarc/actions/runs/33531294046) passed at `5e526bc`; 20 live desktop/tablet/mobile/small-mobile/landscape Playwright/axe scenarios were re-run successfully against deployment `5464a9c8-81ea-4e3a-9761-be69f086af63` at `https://sendarc.pages.dev` on 2026-09-02 | Repeat after the release asset changes download state |
| Dependency/secret/security audit | Verified at pushed head | [Security run 33531293973](https://github.com/KapapiDev/sendarc/actions/runs/33531293973) reproduced the dependency inventories and passed Gitleaks at `5e526bc`; npm/Go vulnerability gates and history scans remain green; [SECURITY_AUDIT.md](SECURITY_AUDIT.md) | Repeat at the immutable release tag |
| `v0.1.0-beta` tag/release | Missing; latest pipeline dry-run verified | Protected [dry-run 33532008202](https://github.com/KapapiDev/sendarc/actions/runs/33532008202) at `5e526bc` built the OAuth-injected unsigned installer, passed 31/31 exact-artifact acceptance cases, generated `SHA256SUMS.txt`, and skipped publication. The downloaded installer independently matched SHA-256 `7c9a974f4c71a0af1185c9e594856cc34c8529ce6faa8a8fd5cffe3f5ce69a9d`; its artifact contained exactly the installer and manifest and the installer was confirmed unsigned; no tag or release exists | Complete real Gmail/acceptance gates, then create the immutable tag and public release |
...
| Real Google OAuth + Gmail Sent | In progress; sign-in, refresh, and credential persistence verified | Project/API, branding/policies, external Testing audience, controlled test user, exact scope, Desktop client, and protected injection are complete. A production-mode Windows build completed real consent, stored the token set in Windows Credential Manager, restored it after restart, passed live Google token introspection, and on 2026-09-02 refreshed an expired access token and persisted the new expiry | Verify disconnect/reconnect, then perform the explicit sanitized send and verify the result in Gmail Sent |
