...
| Windows Credential Manager token storage | Implemented; verification pending | Keyring service `SendArc`, account `oauth-tokens` in `src/app/auth.go`; on 2026-08-30 an opt-in integration test passed against the real Windows Credential Manager for save, fresh-store reload, delete, and `AuthManager` clear. Each run uses a unique test-only service and leaves no credential behind; routine tests cannot touch the production login | Confirm the same isolated test in CI, then complete real Google refresh/disconnect/reconnect evidence in final acceptance |
...
| Requirement | Status | Evidence | Gap / release gate |
|---|---|---|---|
| Current app/frontend tests | Verified at pushed head | [Build and Test run 33284510085](https://github.com/maxtop9843-byte/sendarc/actions/runs/33284510085) passed Go vet, both Go race suites, Svelte check, 77 frontend tests, Wails build, the native-MAPI seven-scenario Windows E2E, all x86/x64 Debug/Release interceptor matrices, and the production-DLL test-hook isolation check at `c57ec2e` | Repeat at the immutable release candidate/tag |
...
| Dependency/secret/security audit | Verified at pushed head | [Security run 33284510072](https://github.com/maxtop9843-byte/sendarc/actions/runs/33284510072) reproduced the checked-in dependency inventories and passed Gitleaks at `c57ec2e`; both npm audits report 0 vulnerabilities, both Go modules report 0 reachable vulnerabilities, and the 595-commit history scan is clean; [SECURITY_AUDIT.md](SECURITY_AUDIT.md) | Repeat at the immutable release tag |
