...
- Clean GitHub Actions at latest code-changing commit `33c75a6` passed the Wails app and x64/x86 Debug/Release interceptors in [33775060019](https://github.com/KapapiDev/sendarc/actions/runs/33775060019), the installer round-trip in [33775060057](https://github.com/KapapiDev/sendarc/actions/runs/33775060057), website/privacy/retention checks in [33775060016](https://github.com/KapapiDev/sendarc/actions/runs/33775060016), and security/policy checks in [33775059985](https://github.com/KapapiDev/sendarc/actions/runs/33775059985). Documentation-only evidence commit `8bb279e` also passed [Build/Test 33879465842](https://github.com/KapapiDev/sendarc/actions/runs/33879465842), [installer 33879465868](https://github.com/KapapiDev/sendarc/actions/runs/33879465868), [website 33879465979](https://github.com/KapapiDev/sendarc/actions/runs/33879465979), and [security 33879465844](https://github.com/KapapiDev/sendarc/actions/runs/33879465844).
...
- The post-Gmail-evidence protected [release dry-run 33881569368](https://github.com/KapapiDev/sendarc/actions/runs/33881569368) repeated the complete release build at `6f4a6aa` without publishing. Its exact 5,898,325-byte installer passed 31/31 acceptance cases. Downloaded artifact `9940295111` contained only the installer and LF-only manifest; independent SHA-256 verification matched `5d8f68c36019357d9afb11ed29512258369b78a717b17c7080652768e9322869`. The unpacked app reports ProductName `SendArc`, CompanyName `장형진`, ProductVersion `0.1.0-beta`; all three unpacked executable/DLL payloads passed the E2E-marker byte scan. Installer/app/DLLs are intentionally unsigned, and no public `v0.1.0-beta` release exists.
...
- On 2026-09-04, the real authenticated Windows app locally previewed and explicitly sent the prepared self-addressed synthetic message through Gmail. The row left the queue, `lastSuccessfulSend` advanced, and Gmail's `in:sent` search showed the matching subject, sanitized body, recipient, and one harmless attachment. No draft endpoint or mailbox-read scope was used.
- Real sign-out revoked and cleared the stored credential and returned the app to its signed-out screen. Reconnect correctly stopped at Google's unverified-app warning instead of bypassing it; the app remains signed out until the verified Production path is completed.
- Google Search Console shows `kapapi.dev` as a Domain property and 장형진 as a verified owner. The OAuth console still has the old `sendarc.pages.dev` URLs, no logo, `sendarc.pages.dev` as the sole authorized domain, and Testing status; these settings were inspected but not falsely recorded as updated.
...
- `docs/OAUTH_VERIFICATION.md` records the exact KaPaPi-hosted public URLs, minimum-scope justification, verified Domain-property ownership, completed runtime send evidence, sanitized demonstration plan, and submission checklist without claiming Production approval or submission.
...
- Google Production verification: save the KaPaPi-hosted branding/policy URLs, `kapapi.dev` authorized domain and logo; complete a post-verification reconnect/consent recording; and submit it without broadening `gmail.send`.
